...
public class DataInitializerTest {
    ...
    void insert() {
        transactionTemplate.executeWithoutResult(status -> {
            ...
            Comment prev = null; // 홀수(i=1,3,5,...)에서 "직전 댓글을 부모로" 대댓글을 만들기 위해 사용
            for (int i = 0; i < BULK_INSERT_SIZE; i++) {
                ...
                Comment comment = Comment.create(
                        snowflake.nextId(),
                        "content",
                        i % 2 == 0 ? null : prev.getCommentId(), // 짝수면 루트 댓글로 만들기 위해 null 전달, 홀수면 직전(prev) 댓글을 부모로 설정해서 대댓글로 생성
                        1L,
                        1L
                );
                prev = comment; // 다음 반복에서 "직전 댓글"로 사용할 수 있게 갱신
